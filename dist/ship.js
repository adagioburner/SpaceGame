...
export class Ship {
    constructor(type, x, y, fieldHeight) {
        ...
        this.y = y;
        this.baseY = y;
        this.driftAmplitude = Math.min(randomRange(4, 16), Math.max(0, (fieldHeight - 2 * type.radius) / 2 - 4));
    ...
    update(dt, speedMultiplier, effects) {
        this.age += dt;
        ...
        if (this.hull < this.maxHull) {
            ...
            if (this.smokeTimer <= 0) {
                ...
                effects.spawnSmoke(this.x - this.radius * 0.35 + randomRange(-4, 4), this.y + randomRange(-this.radius * 0.3, this.radius * 0.3), this.radius * lerp(0.12, 0.24, severity));
            }
        }
    }
    /** Keeps the ship inside the play field after the window is resized. */
    clampToField(fieldHeight) {
...
export function shipSpawnY(fieldHeight, radius) {
