...
/** One ship of the trailing cloud, positioned relative to the spawn point. */
export interface ConvoyMember {
  readonly type: ShipType;
  /** Negative: how far behind the left edge this member starts. */
  readonly dx: number;
  /** Offset from the anchor's lane. */
  readonly dy: number;
}

export interface ConvoyPlan {
  readonly anchorType: ShipType;
  /** Lane the anchor flies down; the cloud is built around it. */
  readonly anchorY: number;
  readonly members: readonly ConvoyMember[];
}

/** Heavies worth building a convoy around, cheapest first. */
const ANCHOR_TYPES: readonly ShipType[] = [SHIP_TYPES.cruiser, SHIP_TYPES.dreadnought];

function pickAnchorType(level: number): ShipType {
  const unlocked = ANCHOR_TYPES.filter((type) => level >= type.unlockLevel);
  if (unlocked.length === 0) return SHIP_TYPES.cruiser;
...
function swarmSize(level: number): number {
...
function pickMemberType(level: number): ShipType {
  const chance = Math.min(
    CONVOY.maxFighterChance,
    (level - CONVOY.fromLevel) * CONVOY.fighterChancePerLevel,
  );
 ...
 */
export function planConvoy(level: number, fieldHeight: number): ConvoyPlan {
  ...
  const anchorY = clamp(
    randomRange(margin, fieldHeight - margin),
    margin,
    Math.max(margin, fieldHeight - margin),
  );
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (let attempt = 0; attempt < 12; attempt++) {
      ...
      if (gap >= CONVOY.minSpacing) break;
    }

    if (best) members.push({ type, dx: best.dx, dy: best.dy });
  }
 ...
 */
export function convoyLaunchDelay(
  anchorType: ShipType,
  fieldWidth: number,
...
export function convoyInterval(level: number): number {
